...
- [x] GAS 核心開發 (`Code.gs`):
    ...
    - [x] **Vibe Check**: 確認按鈕點擊時是否有縮放 (Scale) 的物理回饋感 (active:scale-95)。

## Phase 2: 動態 UI 與 運動風設計 (The Interface) - 已完成
...
- [ ] 動態表單渲染:
    ...
    - [x] 根據 `Config` 表內容，動態在頁尾顯示「聯絡窗口」資訊。
- [ ] 名額即時顯示:
    - [x] 在選擇場次後，顯示「8/12 人」狀態。
    - [ ] 實作「名額進度條」，顏色隨人數增加從綠變紅。

## Phase 3: 互動優化與 Vibe Check (The Polish) - 已完成
加入關鍵的動畫效果與防呆機制，提升系統整體的高級感。
- [ ] 防呆機制:
    - [ ] 限制報名人數下拉選單上限為 5 人。
    - [x] 欄位檢核：姓名、電話、程度、場次為必填 (HTML required)。
- [ ] Vibe 動畫實作:
    - [x] 提交按鈕點擊後的 Loading 狀態。
    - [x] 羽毛球轉場: 實作報名成功後，羽毛球飛過螢幕的動畫效果。
- [ ] 測試與佈署:
    - [x] 測試在高併發（多人同時報名）下的名額更新準確性 (LockService)。
    - [x] 實作 `google.script.run` 呼叫後端 API。
    - [x] 測試佈署為 Web App，並確認手機瀏覽器相容性。
    - [ ] 佈署為 Web App 並提供報名連結。